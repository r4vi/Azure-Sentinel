...
b64service_account_file = os.environ["ServiceAccountFileContent"]
subject = os.environ["Subject"]
json_creds = json.loads(base64.b64decode(b64service_account_file))
...
def get_credentials():
    ...
    with_subject = scoped.with_subject(subject)
    with_subject.refresh(Request())
                    ...
                    if "name" in parameter:
                        for param_name in [
                            ...
                            if param_name in parameter:
                                head_event_part.update(
                                    {parameter["name"]: parameter[param_name]}
                                )
